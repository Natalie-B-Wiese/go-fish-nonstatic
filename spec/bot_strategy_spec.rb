...
RSpec.describe Bot::Strategy::Hard do
  ...
  describe '#record_round_results' do
    ...
    context 'when multiple players' do
      let!(:strategy) { described_class.new }
      let(:player1_name) { 'Henry' }
      let(:player2_name) { 'Bobert' }
      let(:rank1) { '5' }
      let(:rank2) { '3' }
      ...
      let(:round_results) do
        [
          { 'current_player' => player1_name, 'rank' => rank1, 'went_fishing' => false },
          { 'current_player' => player2_name, 'rank' => rank2, 'went_fishing' => false }
        ]
      end
      ...
      it 'creates a key with each the player name' do
        expect(strategy.opponent_hands.key?(player1_name)).to eq true
        expect(strategy.opponent_hands.key?(player2_name)).to eq true
      end

      it 'keys contain only that player card in the hand' do
        expect(strategy.opponent_hands[player1_name]).to include rank1
        expect(strategy.opponent_hands[player1_name]).to_not include rank2

        expect(strategy.opponent_hands[player2_name]).to include rank2
        expect(strategy.opponent_hands[player2_name]).to_not include rank1
      end
    end

    context 'when player is successful (does not go fish)' do
      let!(:strategy) { described_class.new }
      let(:player1_name) { 'Henry' }
      let(:player2_name) { 'Bobert' }
      let(:rank1) { '5' }
      ...
      let(:round_results) do
        [
          { 'current_player' => player2_name, 'rank' => rank1, 'went_fishing' => false },
          { 'current_player' => player2_name, 'rank' => rank2, 'went_fishing' => false },
          { 'current_player' => player2_name, 'rank' => rank3, 'went_fishing' => false },

          { 'current_player' => player1_name, 'rank' => rank2, 'went_fishing' => false }
        ]
      end
      ...
      it 'removes that card rank from all opponents' do
        expect(strategy.opponent_hands[player2_name]).to_not include rank2
      end

      it 'does not remove other cards from the opponents' do
        expect(strategy.opponent_hands[player2_name]).to include rank3
        expect(strategy.opponent_hands[player2_name]).to include rank1
      end

      it 'keeps the card in the successful player hand' do
        expect(strategy.opponent_hands[player1_name]).to include rank2
      end
    end

    context 'when player goes fish (is unsuccessful)' do
      let!(:strategy) { described_class.new }
      let(:player1_name) { 'Henry' }
      let(:player2_name) { 'Bobert' }
      let(:rank1) { '5' }
      ...
      it 'does not remove that card rank from all opponents' do
        expect(strategy.opponent_hands[player2_name]).to include rank2
      end
      ...
      it 'keeps the card in the player hand' do
        expect(strategy.opponent_hands[player1_name]).to include rank2
      end
    end
  end
  ...
  describe '#choose_move' do
    ...
    it 'falls back to picking from the hand when it does not know any opponent hands' do
      strategy = described_class.new
      strategy.opponent_hands = {}
    ...
    it 'ignores round results from the bot itself' do
      ...
      strategy.opponent_hands[bot_name] = ['5']
    ...
    it 'targets an opponent who has a rank the bot holds' do
      strategy = described_class.new
      strategy.opponent_hands['Alice'] = ['5']
    ...
    it 'ignores round results for ranks not in the bot hand' do
      strategy = described_class.new
      strategy.opponent_hands['Alice'] = ['Q']
